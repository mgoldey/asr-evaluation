...
def main(args):
  ...
  if char_level:
    print('CER: {:10.3%} ({:10d} / {:10d})'.format(wer, error_count, ref_token_count))
    print('CRR: {:10.3%} ({:10d} / {:10d})'.format(wrr, match_count, ref_token_count))
  else:
...
def process_line_pair(ref_line, hyp_line, case_insensitive=False, remove_empty_refs=False, char_level=False):
  ...
  if not char_level:
    ...
    ref = list(ref_line.strip())
    hyp = list(hyp_line.strip())
...
def remove_head_id(ref, hyp):
  ...
  if ref_id != hyp_id:
    print(
      'Reference and hypothesis IDs do not match! '
      'ref="{}" hyp="{}"\n'
...
def print_diff(sm, seq1, seq2, prefix1='REF:', prefix2='HYP:', suffix1=None, suffix2=None):
  ...
  for tag, i1, i2, j1, j2 in opcodes:
    ...
    # More complicated logic for a substitution
      ...
      hyp_tokens += s2
  if prefix1:
    ref_tokens.insert(0, prefix1)
  if prefix2:
    hyp_tokens.insert(0, prefix2)
  if suffix1:
    ref_tokens.append(suffix1)
  if suffix2:
    hyp_tokens.append(suffix2)
